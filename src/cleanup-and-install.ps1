...
param(
    [string] $RepoRoot = 'C:\Users\snkjh\Desktop\new_brcm_app'
)
...
if (Test-Path $handleExe) {
    ...
    & $handleExe -a icudtl.dat | Out-String | Write-Host
        ...
        foreach ($pi in $pids) {
            if ($pi) {
                Write-Host ("Stopping process PID {0} ..." -f $pi) -ForegroundColor DarkGray
                try {
                    Stop-Process -Id $pid -Force -ErrorAction Stop
                    Write-Host ("Stopped {0}" -f $pi) -ForegroundColor Green
                } catch {
                    Write-Host ("Failed to stop {0}: {1}" -f $pi, $_) -ForegroundColor Red
                }
            }
        }
